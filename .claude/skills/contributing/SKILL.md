...
- **Descriptions default to nothing.** Apply the deletion test to every comment, JSDoc line, `argTypes` description and story docblock you are about to write: *if I delete this, what does the reader get wrong?* If there is no concrete answer, do not write it. An `argTypes` description is one sentence; a story docblock is usually zero. Never state behaviour that is logically necessary ("counts characters, not markup"), compare the component to another one, describe internal wiring, or restate the name of the thing you are annotating. Over-describing is the single most common note on this repo's reviews — when in doubt, cut it. See [stories.md](references/stories.md) section 8.
