  ...
  describe("matchDate", () => {
    ...
    describe("DateBefore matcher", () => {
      it("returns true when date is strictly before", () => {
        expect(matchDate(day(2026, 4, 14), { before: day(2026, 4, 15) })).toBe(
          true,
        );
      });

      it("returns false when same day", () => {
        expect(matchDate(day(2026, 4, 15), { before: day(2026, 4, 15) })).toBe(
          false,
        );
      });

      it("returns false when after", () => {
        expect(matchDate(day(2026, 4, 16), { before: day(2026, 4, 15) })).toBe(
          false,
        );
      });
    ...
    describe("DateAfter matcher", () => {
      it("returns true when date is strictly after", () => {
        expect(matchDate(day(2026, 4, 16), { after: day(2026, 4, 15) })).toBe(
          true,
        );
      });

      it("returns false when same day", () => {
        expect(matchDate(day(2026, 4, 15), { after: day(2026, 4, 15) })).toBe(
          false,
        );
      });

      it("returns false when before", () => {
        expect(matchDate(day(2026, 4, 14), { after: day(2026, 4, 15) })).toBe(
          false,
        );
      });
    });
