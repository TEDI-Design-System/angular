...
@Component({
  ...
  host: {
    "[class]": "classes()",
    "[style.--_grid-gap]":
      "breakpointInputs().cols === 'auto' ? gridGap() : null",
    "[style.--_grid-col-width]":
      "breakpointInputs().cols === 'auto' ? breakpointInputs().minColWidth + 'px' : null",
    role: "presentation",
  },
})
export class RowComponent implements BreakpointInputs<RowInputs> {
  ...
  classes = computed(() => {
    const classList = [
      "tedi-row",
      `tedi-row--cols-${this.breakpointInputs().cols}`,
    ];
    ...
    if (this.breakpointInputs().alignItems) {
      classList.push(
        `tedi-row--align-items-${this.breakpointInputs().alignItems}`,
      );
    }
