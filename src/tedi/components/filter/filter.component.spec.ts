...
@Component({
  ...
  imports: [FilterComponent, ReactiveFormsModule],
  template: `
    <tedi-filter text="Teenus" [options]="options" [formControl]="control" />
  `,
})
...
@Component({
  standalone: true,
  imports: [FilterComponent, FilterPrependDirective, StatusBadgeComponent],
  template: `
...
describe("FilterComponent", () => {
  ...
  describe("accessibility", () => {
    ...
    it("should have role=dialog on dropdown panel", () => {
      const panel = fixture.debugElement.query(By.css(".tedi-filter-dropdown"));
      expect(panel.nativeElement.getAttribute("role")).toBe("dialog");
    });

    it("should have aria-label on dropdown panel matching text", () => {
      const panel = fixture.debugElement.query(By.css(".tedi-filter-dropdown"));
      expect(panel.nativeElement.getAttribute("aria-label")).toBe("Raviasutus");
    });
    ...
    it("should set aria-disabled on disabled options", () => {
      ...
      expect(items[3].nativeElement.getAttribute("aria-disabled")).toBe("true");
    ...
    it("should set aria-checked=true on select-all when all selected", () => {
      ...
      expect(selectAll.nativeElement.getAttribute("aria-checked")).toBe("true");
    });
    ...
    it("should set aria-activedescendant when option is focused", () => {
      ...
      expect(options.nativeElement.getAttribute("aria-activedescendant")).toBe(
        firstOptionId,
      );
    });
  ...
  describe("keyboard navigation", () => {
    ...
    it("should move to first enabled option on Home", () => {
      ...
      component.onOptionsKeydown(new KeyboardEvent("keydown", { key: "Home" }));
      expect(component.activeOptionIndex()).toBe(0);
    });
    ...
    it("should move to last enabled option on End", () => {
      ...
      component.onOptionsKeydown(new KeyboardEvent("keydown", { key: "End" }));
      expect(component.activeOptionIndex()).toBe(2);
    });
    ...
    it("should toggle focused option on Space", () => {
      ...
      component.onOptionsKeydown(new KeyboardEvent("keydown", { key: " " }));
      expect(component.multiValues()).toEqual(["a"]);
  ...
  describe("custom dropdown content", () => {
    ...
    it("should not show count badge", () => {
      const badge = hostFixture.debugElement.query(By.css("tedi-status-badge"));
      expect(badge).toBeNull();
    });
    ...
    it("should have aria-haspopup on button", () => {
      ...
      expect(button.nativeElement.hasAttribute("aria-haspopup")).toBe(true);
    });
  });
  ...
  describe("prepend directive", () => {
    ...
    it("should keep prepend visible when hideWhenSelected is false", () => {
      const hostFixture = TestBed.createComponent(PrependVisibleHostComponent);
      hostFixture.detectChanges();
  ...
  describe("append directive", () => {
    ...
    it("should keep append visible when selected by default", () => {
      ...
      expect(
        append.nativeElement.classList.contains("tedi-filter__append--hidden"),
      ).toBe(false);
    });
  });
});
