...
export default {
  ...
  argTypes: {
    ...
    options: {
      description:
        "Options for the dropdown. Enables single-select mode, or multiselect when combined with multiselect input",
      control: { type: "object" },
      table: {
        category: "inputs",
...
export const Examples: StoryObj<FilterComponent> = {
  render: () => {
    ...
    const raviasutusControl = new FormControl<string[]>(["ltk", "perh"]);
    ...
    const categoryControl = new FormControl<string[]>([
      "vastuvotud",
      "analuusid",
    ]);
    ...
    return {
      props: {
        ...
        clearAll() {
          [vastuvotudControl, analuusidControl, uuringudControl].forEach((c) =>
            c.setValue(false),
          );
          [uuringControl, teenusControl, aegAlatesControl].forEach((c) =>
            c.setValue(""),
          );
            ...
            if (Array.isArray(val)) {
              for (const v of val) {
                ...
                tags.push({
                  ...
                  remove: () =>
                    f.control.setValue(val.filter((x: string) => x !== v)),
                });
              }
            } else if (typeof val === "boolean" && val) {
              tags.push({
                text: f.label,
                remove: () => f.control.setValue(false),
              });
