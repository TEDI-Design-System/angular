...
const filterablePeople: PersonRecord[] = Array.from(
  { length: 28 },
  (_, index) => {
...
const collapsiblePeople: CollapsibleRecord[] = Array.from(
  { length: 28 },
  (_, index) => {
        ...
        ? [
            {
              ...
              name,
              age: seed.age,
              visits: Math.floor(seed.visits / 2),
              status: "Kehtiv",
            },
            {
              id: `${id}-2`,
              name,
              age: seed.age,
              visits: seed.visits - Math.floor(seed.visits / 2),
              status: "Kehtetu",
            },
          ]
        : undefined;
...
const stickyDoctors: StickyDoctor[] = Array.from(
  { length: 28 },
  (_, index) => ({
...
const serviceSeed: Omit<Service, "id">[] = [
  {
    service: "Vaimse tervise nõustamisteenus",
    doctor: "Pille Paunküla",
    price: 45.5,
    location: "Tallinn",
  },
  {
    service: "Hematoloogia",
    doctor: "Kalle Kuusik",
    price: 89.99,
    location: "Tallinn",
  },
  {
    service: "Ortopeedia",
    doctor: "Märt Männimets",
    price: 110,
    location: "Tallinn",
  },
  {
    service: "Dermatoloogia",
    doctor: "Anna Tamm",
    price: 75,
    location: "Tartu",
  },
  {
    service: "Kardioloogia",
    doctor: "Mati Saar",
    price: 120.5,
    location: "Pärnu",
  },
...
const customDoctors: CustomDoctor[] = Array.from(
  { length: 28 },
  (_, index) => ({
...
const meta: Meta<TediTableStoryArgs> = {
  ...
  argTypes: {
    ...
    placeholder: {
      ...
      table: {
        category: "behavior",
        type: { summary: "string | TemplateRef" },
      },
    },
    state: {
      description:
    ...
    rowClick: {
      description:
        "Emits the activated row. Only fires when `interactive` is true.",
      control: false,
      ...
      rowSpan: groupRowSpan(
        patientRows.map(
          (_, i) =>
            ({
              id: String(i),
              original: patientRows[i],
            }) as unknown as Row<PatientRow>,
        ),
  ...
  template: `
    ...
    >
      <p style="margin: 0 0 8px;">
        ...
        entire header title becomes clickable, an icon reflects the sort state,
        and <code>aria-sort</code> is wired automatically.
      </p>
      <p style="margin: 0 0 8px;">
      ...
      <ul style="margin: 0 0 8px; padding-left: 20px;">
        ...
        <li>
          ...
          &lt;0, 0, or &gt;0 just like <code>Array.sort</code>. The table flips
          the sign on descending sort, so always write the comparator for
          ascending order.
        </li>
      </ul>
      <p style="margin: 0;">
        ...
        <code>'alphanumeric'</code>), the rest use the inferred default.
      </p>
    </div>
        ...
        @for (option of certStatuses; track option) {
          <label class="tedi-filters-story__option">
            <input
              ...
              (change)="
                ctx.setValue(
                  toggleStatus(ctx.value, option, $any($event.target).checked)
                )
              "
            />
            <span>{{ option }}</span>
...
@Component({
  ...
  template: `
    <p style="margin-bottom: 10px;">
      {{
        active() ? "You clicked " + active()!.name : "Click a row to select it."
      }}
    </p>
    <tedi-table
      #table
      ...
      <span style="display: inline-block; max-width: 480px;">
        @if (expanded) {
          ...
          <a
            ...
            href="#"
            (click)="
              toggleDescription(ctx.row.original.id); $event.preventDefault()
            "
          >
            Näita vähem
          </a>
    ...
    >
      ...
      <ul style="margin: 0; padding-left: 20px;">
        ...
        <li>
          ...
          <code>Space</code>/<code>Enter</code> to drop, <code>Escape</code> to
          cancel.
        </li>
      </ul>
    </div>
    ...
    >
      ...
      <ul style="margin: 0 0 8px; padding-left: 20px;">
        ...
        <li>
          ...
          <code>Space</code>/<code>Enter</code> to pick it up,
          <code>←</code>/<code>→</code> to move, <code>Space</code>/<code
            >Enter</code
          >
          to drop, <code>Escape</code> to cancel.
        </li>
      </ul>
