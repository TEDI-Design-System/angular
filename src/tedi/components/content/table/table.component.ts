...
export class TediTableComponent<TData> {
  ...
  protected readonly rowDetailsLabel =
    this.translation.track("table.row-details");
  ...
  private readonly paginationOptions =
    computed<ResolvedPaginationOptions | null>(() => {
  ...
  protected readonly resolvedBottomSlot =
    computed<ResolvedPaginationSlot | null>(() => {
  ...
  protected readonly hostClasses = computed(() => {
    ...
    const flags: [boolean, string][] = [
      ...
      [
        this.reorderableRows() || this.reorderableColumns(),
  ...
  protected handleRowReorderKeydown(
    event: KeyboardEvent,
    row: Row<TData>,
  ): void {
    ...
    this.applyPatch<ColumnOrderState>(
      (prev) => {
        const fullOrder =
          prev.length > 0
            ? [...prev]
            : untracked(() => this.table.getAllLeafColumns().map((c) => c.id));
  ...
  protected executeColumnReorder(
    ...
    const col = this.table.getVisibleLeafColumns().find((c) => c.id === moved);
