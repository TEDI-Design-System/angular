...
const TRANSLATIONS: Record<string, Translator> = {
  ...
  "table.row-reorder.move": (pos) =>
    `Row moved to position ${pos ?? ""}`.trim(),
...
class HostComponent {
  readonly subTemplateRef =
    viewChild<TemplateRef<{ $implicit: Row<Person> }>>("sub");

  readonly data = signal<Person[]>(data);
  readonly columns = signal<TediColumnDef<Person>[]>(columns);
...
describe("TediTableComponent", () => {
  ...
  describe("selection", () => {
    ...
    it("marks the selection control cells with tedi-table__cell--control", () => {
      ...
      const selectHeader = fixture.nativeElement
        ...
        ?.closest("th");
      expect(
        selectHeader?.classList.contains("tedi-table__cell--control"),
      ).toBe(true);
    });
  ...
  describe("expansion", () => {
    ...
    it("marks the icon-only expand cell with tedi-table__cell--control", () => {
      ...
      const expandHeader = fixture.nativeElement
        ...
        ?.closest("td");
      expect(
        expandHeader?.classList.contains("tedi-table__cell--control"),
      ).toBe(true);
    });
    ...
    it("indexes root rows sequentially and leaves sub-rows without aria-rowindex", () => {
      ...
      expect(
        subRows.every((r) => r.getAttribute("aria-rowindex") === null),
      ).toBe(true);
    });
  });
  ...
  describe("pagination", () => {
    ...
    it("renders only the bottom paginator by default", () => {
      ...
      expect(
        host?.classList.contains("tedi-table--has-pagination-bottom"),
      ).toBe(true);
    ...
    it("forwards visual options to the bottom paginator", () => {
      ...
      expect(
        paginator?.classList.contains("tedi-pagination--bg-transparent"),
      ).toBe(true);
      expect(
        paginator?.classList.contains("tedi-pagination--divider-none"),
      ).toBe(true);
    });
    ...
    it("syncs state between top and bottom paginators", () => {
      ...
      fixture.componentInstance.paginationTop.set(true);
      fixture.detectChanges();
      const paginators = fixture.debugElement.queryAll(
        By.css("tedi-pagination"),
      );
  ...
  describe("clickable rows", () => {
    ...
    it("applies the rowAriaLabel resolver as aria-label on interactive rows", () => {
      ...
      expect(firstRow?.getAttribute("aria-label")).toBe(`Open ${data[0].name}`);
    });
  ...
  describe("filterable shorthand", () => {
    ...
    it("commits the draft to column.setFilterValue on Apply and closes the popover", () => {
      ...
      expect(
        triggerAfter?.classList.contains("tedi-table-header-button--selected"),
      ).toBe(true);
    });
    ...
    it("reflects active state with selected + filled classes on the trigger", () => {
      ...
      expect(
        trigger.classList.contains("tedi-table-header-button--selected"),
      ).toBe(false);
      // Apply.
      column.setFilterValue("Anna");
      fixture.detectChanges();
...
describe("Table: filter modal", () => {
  ...
  function setup(
    current: Breakpoint,
  ): ComponentFixture<FilterModalHostComponent> {
    modalOpen = jest.fn();
...
describe("Table: keyboard column reordering", () => {
  ...
  class ReorderTestHostComponent {
    ...
    pagination = signal<boolean | TablePaginationOptions | undefined>(
      undefined,
    );
  }
  ...
  function _createFixture() {
    ...
    ) as ComponentFixture<ReorderTestHostComponent>;
    return fixture;
  }
  ...
  describe("pickup (Space/Enter)", () => {
    it("should set aria-pressed on picked-up cell", async () => {
      ...
      const pickedHandle = pickedCell?.querySelector(
        ".tedi-table__drag-handle",
      );
  ...
  describe("move (ArrowLeft/ArrowRight)", () => {
    it("should update move delta when arrows pressed", async () => {
      ...
      // Move right twice
      const rightEvent1 = new KeyboardEvent("keydown", {
        key: "ArrowRight",
        bubbles: true,
      });
      ...
      const rightEvent2 = new KeyboardEvent("keydown", {
        key: "ArrowRight",
        bubbles: true,
      });
  ...
  describe("ARIA accessibility", () => {
    ...
    it("should add visual highlight class to picked-up cell", async () => {
      ...
      expect(
        cells[0]?.classList.contains("tedi-table__header-cell--picked-up"),
      ).toBe(true);
