...
describe("CalendarDayGridComponent", () => {
  ...
  describe("daySelect", () => {
    ...
    it("does not emit when a disabled cell is clicked", () => {
      fixture.componentRef.setInput("disabledMatchers", [
        new Date(2024, 4, 15),
      ]);
      fixture.detectChanges();
  ...
  describe("range mode tolerance", () => {
    it("treats a Date value as no-range for range-mode rendering", () => {
      ...
      expect(
        btn?.classList.contains("tedi-calendar-day-grid__day--range-start"),
      ).toBe(false);
    });
  ...
  describe("availableDays as predicate function", () => {
    it("disables days that fail the predicate", () => {
      ...
      expect(
        buttonForDay(new Date(2024, 4, 15))?.getAttribute("aria-disabled"),
      ).toBeNull();
      expect(
        buttonForDay(new Date(2024, 4, 16))?.getAttribute("aria-disabled"),
      ).toBe("true");
    });
  });
  ...
  describe("weekNumber()", () => {
    ...
    it("returns null for an all-null row", () => {
      expect(
        component.weekNumber([null, null, null, null, null, null, null]),
      ).toBeNull();
    });
  });
  ...
  describe("a11y", () => {
    ...
    it("exposes a long aria-label on each weekday header", () => {
      ...
      const labels = headers.map((h) =>
        (h.nativeElement as HTMLElement).getAttribute("aria-label"),
      );
    ...
    it("week number cell is a <th> rowheader with translated aria-label and visible number", () => {
      ...
      expect(
        (cell.nativeElement.textContent as string).trim().length,
      ).toBeGreaterThan(0);
    });
    ...
    it("week number column header has scope=col and a visually hidden translated label", () => {
      ...
      const srOnly = header.nativeElement.querySelector(
        ".sr-only",
      ) as HTMLElement;
