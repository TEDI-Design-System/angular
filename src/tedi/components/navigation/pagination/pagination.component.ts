...
export class PaginationComponent {
  ...
  protected readonly nextItem = computed(
    () =>
      ...
      PaginationComponent.EMPTY_NAV_ITEM,
  );
  ...
  protected readonly mergedLabels = computed<PaginationLabels>(() => {
    ...
    return {
      ...
      pageAriaLabel:
        overrides.pageAriaLabel ??
        ((p) => t.translate("pagination.page", p, false)),
      currentPageAriaLabel:
        overrides.currentPageAriaLabel ??
        ((p) => t.translate("pagination.page", p, true)),
      results:
        overrides.results ??
        ((count) => t.translate("pagination.results", count)),
  ...
  protected openPageSizePicker(): void {
    const labels = this.mergedLabels();
    const current = this.pageSize();
    const options: PaginationOptionPickerOption[] =
      this.pageSizeSelectOptions().map(({ value, label }) => ({
        value,
        label,
