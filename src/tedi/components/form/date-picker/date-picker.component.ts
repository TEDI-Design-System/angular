...
export class DatePickerComponent implements OnInit, ControlValueAccessor {
  ...
  readonly fieldDisabled = computed(
    () => this.inputDisabled() || this.formDisabled(),
  );
  ...
  readonly calendarGrid =
    viewChild<DatePickerCalendarGridComponent>("gridElement");
  ...
  private matches(m: DatePickerMatcher, date: Date): boolean {
    ...
    if ("from" in m) {
      ...
      if (to) {
        return (
          (isSameDay(date, from) || isAfterDay(date, from)) &&
  ...
  private findNextEnabledMonth(): Date | null {
    ...
    const years = this.years();
    const maxYear =
      years.length > 0 ? years[years.length - 1] : current.getFullYear() + 20;
