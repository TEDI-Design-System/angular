...
export default {
  ...
  argTypes: {
    ...
    hideEditIcon: {
      description:
        "Hides the edit icon. Use only when another visible cue identifies the value as editable.",
    ...
    fullWidth: {
      description:
        "Makes the read trigger and editor fill their container, with the icon at the far edge.",
      control: { type: "boolean" },
      table: {
        category: "inputs",
        type: { summary: "boolean" },
 ...
 * Use `hideEditIcon` only when another visible cue communicates this.
 * `fullWidth` fills the container and places the icon at the far edge.
 */
export const EditIcon: StoryObj<InlineEditComponent> = {
  render: () => ({
    props: {
      rows: [
        { name: "Default", hide: false, full: false },
        { name: "Full width", hide: false, full: true },
        { name: "Hidden", hide: true, full: false },
      ],
    },
    template: `
      <div style="display: grid; gap: 8px;">
