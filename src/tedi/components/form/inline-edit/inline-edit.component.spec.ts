...
describe("InlineEditComponent", () => {
  ...
  describe("read mode", () => {
    it("shows the display value on a button", () => {
      ...
      expect(
        trigger()!
          .querySelector(".tedi-inline-edit__value")!
          .textContent?.trim(),
      ).toBe("Mari Maasikas");
    });
    ...
    it("shows the edit icon by default and hides it on request", () => {
      expect(dom().querySelector("tedi-icon")).toBeTruthy();

      host.hideEditIcon.set(true);
      fixture.detectChanges();
  ...
  describe("host classes", () => {
    ...
    it("reflects full width only when it is turned on", () => {
      expect(root().classList).not.toContain("tedi-inline-edit--full-width");

      host.fullWidth.set(true);
      fixture.detectChanges();

      expect(root().classList).toContain("tedi-inline-edit--full-width");
    });
  });
  ...
  it("commits when focus moves to an overlay that was already open", async () => {
    // A modal hosting the field, say — it predates the edit, so it is not ours.
    const pane = addPane();
    await enterEditMode();

    editorInput()!.dispatchEvent(
      new FocusEvent("focusout", { bubbles: true, relatedTarget: pane }),
    );
    fixture.detectChanges();
  ...
  it("stays open when the control opens an overlay during the edit", async () => {
    // tedi-date-field's calendar links its pane with neither aria-controls nor
    // aria-owns, so only the "opened after we started" rule keeps it open.
    await enterEditMode();
    const pane = addPane();
