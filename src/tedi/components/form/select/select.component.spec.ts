      ...
      @if (useOptionTemplate) {
        <ng-template tediSelectOption let-item let-selected="selected">
          <span class="custom-option"
            >{{ item.name }} -
            {{ selected ? "selected" : "not selected" }}</span
          >
        </ng-template>
      }
      @if (useValueTemplate) {
...
describe("SelectComponent", () => {
  ...
  const getTrigger = () =>
    hostEl.querySelector(".tedi-select__trigger") as HTMLElement;
  const getSearchInput = () =>
    hostEl.querySelector(".tedi-select__search-input") as HTMLInputElement;
  const getLabel = () => hostEl.querySelector("[tedi-label]") as HTMLElement;
  const getClearButton = () =>
    hostEl.querySelector(".tedi-select__clear") as HTMLButtonElement;
  const getDropdown = () =>
    document.querySelector(".tedi-select__dropdown") as HTMLElement;
  const getOptions = () =>
    Array.from(
      document.querySelectorAll(
        ".tedi-dropdown-item:not(.tedi-select__group-name):not(.tedi-select__no-options)",
      ),
    ) as HTMLElement[];
  ...
  const getTags = () =>
    Array.from(hostEl.querySelectorAll("tedi-tag")) as HTMLElement[];
  ...
  describe("Multiple select", () => {
    ...
    it("forwards tagEllipsis to the rendered tags", fakeAsync(() => {
      ...
      expect(
        tags.every((t) => t.classList.contains("tedi-tag--ellipsis")),
      ).toBe(true);
    }));
    ...
    it("should deselect via tag close button when clearableTags=true", fakeAsync(() => {
      ...
      const tags = getTags();
      const closeBtn = tags[0].querySelector(
        "[tedi-closing-button]",
      ) as HTMLElement;
      closeBtn?.click();
      fixture.detectChanges();
  ...
  describe("Grouped options", () => {
    ...
    it("should select group options when selectableGroups=true", fakeAsync(() => {
      ...
      const groupHeaders = document.querySelectorAll(
        ".tedi-select__group-name--selectable",
      );
      expect(groupHeaders.length).toBe(2);
  ...
  describe("Custom templates", () => {
    ...
    it("getOptionContext should return correct context", () => {
      ...
      const context = select.getOptionContext(
        option as SelectOption<unknown>,
        0,
      );
  ...
  describe("Computed properties", () => {
    it("dropdownPositions anchor to the start edge by default", () => {
      expect(
        select.dropdownPositions().every((p) => p.originX === "start"),
      ).toBe(true);
      expect(
        select.dropdownPositions().every((p) => p.overlayX === "start"),
      ).toBe(true);
    });
    ...
    it("dropdownPositions anchor to the end edge when dropdownAlign is 'end'", () => {
      ...
      expect(
        select.dropdownPositions().every((p) => p.overlayX === "end"),
      ).toBe(true);
    });
  ...
  describe("Helper methods", () => {
    ...
    it("isGroupIndeterminate should return false for non-existent group", fakeAsync(() => {
      host.items = [{ id: 1, name: "A1", group: "A" }];
      host.bindLabel = "name";
      host.bindValue = "id";
      host.groupBy = "group";
      host.allowMultiple = true;
  ...
  describe("label tooltip", () => {
    ...
    it("should not open the dropdown when clicking the tooltip info button", fakeAsync(() => {
      ...
      const infoButton = hostEl.querySelector(
        "[tedi-info-button]",
      ) as HTMLElement;
      infoButton.click();
      fixture.detectChanges();
  ...
  describe("template context guards", () => {
    it("SelectOptionTemplateDirective ngTemplateContextGuard should return true", () => {
      expect(
        SelectOptionTemplateDirective.ngTemplateContextGuard(
          {} as SelectOptionTemplateDirective,
          {},
        ),
      ).toBe(true);
    });
    ...
    it("SelectValueTemplateDirective ngTemplateContextGuard should return true", () => {
      expect(
        SelectValueTemplateDirective.ngTemplateContextGuard(
          {} as SelectValueTemplateDirective,
          {},
        ),
      ).toBe(true);
    });
