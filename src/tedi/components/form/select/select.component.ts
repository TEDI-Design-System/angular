...
export type VirtualRow<T = unknown> =
  { kind: "select-all" } | { kind: "option"; option: SelectOption<T> };
...
export class SelectComponent<T = unknown>
  implements AfterContentChecked, AfterViewChecked, ControlValueAccessor
{
  /**
  ...
  private readonly selectedValueSet = computed(
  ...
  readonly virtualize = computed(
    () =>
      this.virtualScroll() && this.dropdownType() === "menu" && !this.groupBy(),
  );
  ...
  readonly virtualViewportHeight = computed(() => {
    ...
    const available =
      (this.dropdownMaxHeight() ?? rowHeight * 10) - selectAllHeight;
  ...
  private getAvailableTagWidth(): number {
    ...
    const padding =
      (parseFloat(triggerStyle.paddingLeft) || 0) +
      (parseFloat(triggerStyle.paddingRight) || 0);

    let nonTagWidth = 0;
    const arrow: HTMLElement | null = trigger.querySelector(
      ".tedi-select__arrow",
    );
    const clear: HTMLElement | null = trigger.querySelector(
      ".tedi-select__clear",
    );
    const searchInput: HTMLElement | null = trigger.querySelector(
      ".tedi-select__search-input",
    );
    if (arrow)
      nonTagWidth +=
        arrow.offsetWidth +
        (parseFloat(getComputedStyle(arrow).marginLeft) || 0) +
        (parseFloat(getComputedStyle(arrow).paddingLeft) || 0);
