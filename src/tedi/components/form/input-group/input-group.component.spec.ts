...
describe("InputGroupComponent", () => {
  ...
  it("adds has-prefix / has-suffix modifiers when slots are present", () => {
    ...
    expect(root().classList.contains("tedi-input-group--has-prefix")).toBe(
      true,
    );
    expect(root().classList.contains("tedi-input-group--has-suffix")).toBe(
      true,
    );
  });
  ...
  it("applies the text modifier for a text-only addon", () => {
    ...
    expect(prefix.classList.contains("tedi-input-group__prefix--text")).toBe(
      true,
    );
  });
...
describe("InputGroupComponent with a tedi-label-row", () => {
  ...
  it("projects the label row above the control row", () => {
    ...
    expect(
      labelRow.compareDocumentPosition(row) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
  });
});
