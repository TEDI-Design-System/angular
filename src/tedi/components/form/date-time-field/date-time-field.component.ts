...
@Component({
  ...
  providers: [
    ...
    },
    {
      ...
      useFactory: (field: DateTimeFieldComponent) => field.childContext,
      deps: [forwardRef(() => DateTimeFieldComponent)],
    },
  ],
  host: {
    ...
    "[class.tedi-date-time-field--small]": "resolvedSize() === 'small'",
    "[class.tedi-date-time-field--large]": "resolvedSize() === 'large'",
  },
})
export class DateTimeFieldComponent
  implements OnInit, ControlValueAccessor, FormFieldControl<DateTimeFieldValue>
{
   ...
   * set here.
   */
  readonly size = input<DateTimeFieldSize | undefined>();
  /**
  ...
  constructor() {
    effect(() => {
      const anchor =
        this.deriveAnchor(this.value()) ?? this.initialMonth() ?? null;
