...
describe("DateTimeFieldComponent", () => {
  ...
  describe("opening the picker", () => {
    it("opens the popover on icon click", () => {
      ...
      el.querySelector<HTMLButtonElement>(".tedi-date-input__icon")!.click();
      fixture.detectChanges();
  ...
  describe("overlay dismissal", () => {
    ...
    it("closes on an outside click but not an inside one", () => {
      ...
      Object.defineProperty(inside, "target", {
        value: el.querySelector("input"),
      });
  ...
  describe("ControlValueAccessor + FormFieldControl", () => {
    ...
    it("the invalid input toggles the invalid signal", () => {
      const { component, fixture } = createField();
    ...
    it("paints its own surface when it stands alone", () => {
      const { component, el } = createField();
      expect(component.paintsSurface()).toBe(true);
      expect(el.classList.contains("tedi-field-surface")).toBe(true);
    });
    ...
    it("focus does nothing when the field is disabled", () => {
      const { component, el } = createField({ inputDisabled: true });
      const input = el.querySelector(
        ".tedi-date-input__input",
      ) as HTMLInputElement;
      const focusSpy = jest.spyOn(input, "focus");
    ...
    it("reset does nothing when the field is read-only", () => {
      const value = new Date(2025, 8, 1, 10, 0);
      const { component } = createField({ value, readOnly: true });
      component.reset();
    ...
    it("integrates with a reactive FormControl", () => {
      ...
      expect((fixture.componentInstance.control.value as Date).getHours()).toBe(
        9,
      );
    });
  });
  ...
  describe("range time headings", () => {
    ...
    it("keeps the slot grid for both ends when availableTimes is set", () => {
      ...
      fixture.detectChanges();
      expect(document.querySelector("tedi-time-field")).toBeNull();
      expect(document.querySelectorAll(".tedi-time-picker--slots").length).toBe(
        2,
      );
      component.closeOverlay();
      fixture.detectChanges();
