...
describe("TediTableComponent", () => {
  ...
  describe("grouping", () => {
    ...
    it("spans a groupBy:true column across consecutive rows of a group", () => {
      ...
      expect(bodyRows[2].querySelector("td")?.getAttribute("rowspan")).toBeNull();
    });
    ...
    it("draws group dividers only between groups when rowGroupDividers='between'", () => {
      const fixture = setupGrouped((host) => host.rowGroupDividers.set("between"));
      expect(fixture.nativeElement.querySelector(".tedi-table")?.className).toContain(
        "tedi-table--group-dividers-between",
      );
    ...
    it("orders the control columns per controlColumnOrder", () => {
      ...
      expect(headerCells[1].querySelector('input[type=checkbox]')).not.toBeNull();
    });
  });
