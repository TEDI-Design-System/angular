...
describe("EmptyStateComponent", () => {
  ...
  @Component({
    standalone: true,
    imports: [EmptyStateComponent],
    template: `<tedi-empty-state heading="Choose new time">Empty</tedi-empty-state>`,
  })
  ...
  it("applies the separate type class by default", () => {
    ...
    expect(root.nativeElement.className).toContain("tedi-empty-state--separate");
  });
  ...
  ] as const).forEach(([type, fragment]) => {
    @Component({
      standalone: true,
      imports: [EmptyStateComponent],
  ...
  ] as const).forEach(([size, fragment]) => {
    @Component({
      standalone: true,
      imports: [EmptyStateComponent],
